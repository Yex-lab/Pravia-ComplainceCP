...
export function HeaderSection({
  ...
  return (
    <HeaderRoot
      ...
      isOffset={isOffset}
      bgOpacity={bgOpacity}
      bgTransitionOpacity={bgTransitionOpacity}
      bgVisible={bgVisible}
...
type HeaderRootProps = Pick<HeaderSectionProps, 'disableOffset' | 'disableElevation' | 'bgOpacity' | 'bgTransitionOpacity' | 'bgVisible'> & {
  isOffset: boolean;
};

const HeaderRoot = styled(AppBar, {
  shouldForwardProp: (prop: string) =>
    !['isOffset', 'disableOffset', 'disableElevation', 'bgOpacity', 'bgTransitionOpacity', 'bgVisible', 'sx'].includes(prop),
})<HeaderRootProps>(({ isOffset, disableOffset, disableElevation, bgOpacity = 1, bgTransitionOpacity = 0.6, bgVisible = true, theme }) => {
  const pauseZindex = { top: -1, bottom: -2 };

  // Use the appropriate opacity based on scroll state
  const currentOpacity = isOffset ? bgTransitionOpacity : bgOpacity;

  const bgStyles: CSSObject = {
    ...theme.mixins.bgBlur({
      color: varAlpha(theme.vars.palette.background.defaultChannel, currentOpacity),
    }),
    content: '""',
    position: 'absolute',
    top: 0,
