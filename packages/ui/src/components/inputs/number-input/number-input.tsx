  ...
  return (
    <Box {...slotProps?.wrapper}>
      <NumberInputRoot
        sx={[
          (theme) => ({
            '--border-color': varAlpha(theme.palette.grey[500], 0.2),
            '--vertical-divider-color': hideDivider
              ? 'transparent'
              : varAlpha(theme.palette.grey[500], 0.2),
            '--input-background':
              !disabled && error
                ? varAlpha(theme.palette.error.main, 0.08)
                : varAlpha(theme.palette.grey[500], 0.08),
          }),
