...
export const FILE_ICONS: Record<FileFormat | 'folder' | 'unknown', string> = {
  txt: 'solar:document-text-bold',
  zip: 'solar:folder-with-files-bold',
  ...
  excel: 'solar:chart-bold',
  unknown: 'solar:file-bold',
  folder: 'solar:folder-bold',
  photoshop: 'solar:palette-bold',
  illustrator: 'solar:pallete-2-bold',
  powerpoint: 'solar:presentation-graph-bold',
};
...
/**
 * Returns the corresponding icon name based on the file format.
 *
 * @example getFileIcon('file.pdf') => 'solar:document-bold'
 * @example getFileIcon('image.png') => 'solar:gallery-bold'
 */
export function getFileIcon(input?: FileInput): string {
  const format = detectFileFormat(input);
  return FILE_ICONS[format] || FILE_ICONS.unknown;
}
